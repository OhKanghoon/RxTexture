//
//  XCTest+Controls.swift
//  RxTextureTests
//
//  Created by ohkanghoon on 22/11/2019.
//
...
extension XCTest {
  ...
  func ensurePropertyDeallocated<C, T>(
    ...
    XCTWaiter().wait(for: [completeExpectation, deallocateExpectation], timeout: 3.0, enforceOrder: false)
  ...
  func ensureEventDeallocated<C, T>(
    _ createControl: @escaping () -> C,
    file: StaticString = #file,
    line: UInt = #line,
    _ eventSelector: (C) -> ControlEvent<T>
  ) where C: NSObject {
    return ensureEventDeallocated({ () -> (C, Disposable) in (createControl(), Disposables.create()) }, file: file, line: line, eventSelector)
  }

  func ensureEventDeallocated<C, T>(
    _ createControl: () -> (C, Disposable),
    file: StaticString = #file,
    line: UInt = #line,
    _ eventSelector: (C) -> ControlEvent<T>
  ) where C: NSObject {
  ...
  func ensureControlObserverHasWeakReference<C, T>(
    file: StaticString = #file,
    line: UInt = #line,
    _ createControl: @autoclosure() -> (C),
    _ observerSelector: (C) -> AnyObserver<T>,
    _ observableSelector: () -> (Observable<T>)
  ) where C: NSObject {
    var deallocated = false
